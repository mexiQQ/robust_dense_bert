...
def load_data(tokenizer, args):
    ...
    if args.dataset_name == 'imdb' or args.dataset_name == 'ag_news':
        ...
        train_loader = DataLoader(train_dataset, batch_size=args.bsz, shuffle=True, collate_fn=collator)
        dev_dataset = Huggingface_dataset(args, tokenizer, name_or_dataset=args.dataset_name, subset=args.task_name, split='validation')
        dev_loader = DataLoader(dev_dataset, batch_size=args.eval_size, shuffle=False, collate_fn=collator)
        test_loader = dev_loader
        ...
        for model_inputs, labels in pbar:
            ...
            if False: #args.adv_init_mag > 0:
                ...
                if args.adv_norm_type == 'l2':
                    ...
                    dims = embedding_init.size(-1)
                    magnitude = args.adv_init_mag / math.sqrt(dims)
            ...
            for astep in range(args.adv_steps):
                ...
                if args.adv_norm_type == "l2":
                    ...
                    denorm = torch.norm(delta_grad, dim=-1, keepdim=True)
                    ...
                    denorm = torch.norm(delta_grad, dim=-1, p=float("inf"), keepdim=True)
                    denorm = torch.clamp(denorm, min=1e-8)
                    delta = (delta + args.adv_lr * delta_grad / denorm).detach()
